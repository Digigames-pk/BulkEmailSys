...
export default function EditCampaign({ campaign, emailTemplates, groups }: EditCampaignProps) {
    const { t } = useTranslation();
    const [selectedTemplate, setSelectedTemplate] = useState<EmailTemplate | null>(
        emailTemplates.find(t => t.id === campaign.email_template_id) || null
    );
    const [selectedGroup, setSelectedGroup] = useState<Group | null>(
        groups.find(g => g.id === campaign.group_id) || null
    );
        ...
        <AppLayout>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold">{t('edit_campaign')}</h1>
                        <p className="text-muted-foreground">{t('update_email_campaign')}</p>
                    </div>
                </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{t('update_campaign_information')}</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex items-center space-x-4 pt-4">
                                <Button type="submit" disabled={processing}>
                                    {processing ? t('updating') : t('update_campaign')}
                                </Button>
