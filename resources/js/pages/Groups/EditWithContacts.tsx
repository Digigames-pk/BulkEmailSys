...
interface EditWithContactsProps {
    group: Group;
    contactsUrl: string;
}

export default function EditWithContacts({ group, contactsUrl }: EditWithContactsProps) {
    ...
    useEffect(() => {
        ...
        setSelectedContacts(group.contacts.map(contact => contact.id));
        setData('contact_ids', group.contacts.map(contact => contact.id));
        fetchAvailableContacts(1, true);
    }, [group.contacts, fetchAvailableContacts, setData]);
        ...
        <AppLayout>
            ...
            <div className="space-y-8">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">{t('edit_group')}</h1>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3 max-h-80 overflow-y-auto">
                                {availableContacts.map((contact) => (
                                    <div
                                        key={contact.id}
                                        className={`relative p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${selectedContacts.includes(contact.id)
                            ...
                            <div className="flex items-center space-x-4 pt-6 border-t">
                                <Button
                                    ...
                                    className="px-8"
                                >
                                    {processing ? t('updating') : t('update_group')} ({selectedContacts.length})
                                </Button>
